...
static void timer_function(struct timer_list *unused)
{
	...
	// printk("The gpio of dev num is:%d\n", dev->led_gpio);
	...
	// mod_timer(&start_timer, jiffies + msecs_to_jiffies(timerdev.timeperiod)); 
...
static int led_init(void)
{
	...
	timerdev.led_gpio = of_get_named_gpio(timerdev.nd ,"led-gpio", 0); /*设备杰迪纳， GPIO信息的属性名，多个索引，只有一个GPIO时为0*/
	...
	/* 初始化led所使用的IO */
	gpio_request(timerdev.led_gpio, "led");		/* 请求IO 	*/ /*申请一个GPIO管脚，第一个参数用 of_get_named_gpio
	 从 设备树获取指定GPIO属性信息*/
...
static long timer_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case OPEN_CMD:		/* 打开定时器 */
			...
			// DEFINE_TIMER(dev->timer, timer_function1);
...
static int __init timer_init(void)
{
	...
	// /* 使用 DEFINE_TIMER 的宏进行改装*/
	timerdev.timer = start_timer;
	printk("timerdev init complete!");
	return 0;
}
